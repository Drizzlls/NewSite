(() => {
    ...
    function initSliders() {
        if (document.querySelector(".cases-main__body")) new swiper_core_Swiper(".cases-main__body", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            speed: 800,
            navigation: {
                prevEl: ".cases-main-btn-prev",
                nextEl: ".cases-main-btn-next"
            },
            on: {}
        });
        if (document.querySelector(".blog-main__body")) new swiper_core_Swiper(".blog-main__body", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 1.1,
            ...
            speed: 800,
            navigation: {
                prevEl: ".blog-main-btn-prev",
                nextEl: ".blog-main-btn-next"
            },
            breakpoints: {
                640: {
                    slidesPerView: 2,
        ...
        if (document.querySelector(".news__body")) {
            ...
            resizableSwiper("(min-width: 639.98px)", ".news__body", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 2,
        ...
        if (document.querySelector(".sale-page__body")) {
            const resizableSwiper = (breakpoint, swiperClass, swiperSettings, callback) => {
                let swiper;
                breakpoint = window.matchMedia(breakpoint);
                const enableSwiper = function(className, settings) {
            ...
            resizableSwiper("(min-width: 479.98px)", ".sale-page__body", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
...
***************************************************************************** */
    ...
    if (document.querySelector(".quiz")) {
        document.addEventListener("click", quizActions);
        let curStep = 1;
        quiz(curStep);
        function quizActions(e) {
            const targetElement = e.target;
            if (targetElement.closest(".quiz-step-prev")) {
                curStep > 1 ? curStep-- : null;
                quiz(curStep);
            }
            if (targetElement.closest(".quiz-step-next")) {
                curStep < 6 ? curStep++ : null;
                quiz(curStep);
            }
        }
    }
    function documentActions(e) {
        const targetElement = e.target;
        if (targetElement.closest(".city-header__link")) targetElement.classList.add("_active");
        if (targetElement.closest(".search-header__btn")) {
            const searchBox = document.querySelector(".search-box");
            searchBox.classList.toggle("_active");
            targetElement.classList.toggle("_active");
        }
        if (targetElement.closest(".select_city .select__option")) trackContentChangeOnClick(targetElement);
    }
    document.addEventListener("click", documentActions);
    document.addEventListener("click", (function(event) {
        const element = document.querySelector(".header-top__city");
        deleteActiveClass(element);
    }));
    ...
    function deleteActiveClass(element) {
        if (event.target !== element && !element.contains(event.target)) {
            const activeLink = document.querySelector(".city-header__link._active");
            activeLink ? activeLink.classList.remove("_active") : null;
        }
    }
    function quiz(step) {
        const contentQuiz = document.querySelectorAll(".content-quiz"), btnWrapper = document.querySelector(".quiz__actions"), btnPrev = document.querySelector(".quiz-step-prev"), btnNext = document.querySelector(".quiz-step-next"), activeItem = document.querySelector(".content-quiz-active");
        activeItem ? activeItem.classList.remove("content-quiz-active") : null;
        contentQuiz.forEach((item => {
            item.hidden = true;
        }));
        btnPrev.style.display = "inline-flex";
        btnNext.style.display = "inline-flex";
        contentQuiz[step - 1].hidden = false;
        contentQuiz[step - 1].classList.add("content-quiz-active");
        if (step < 2) btnPrev.style.display = "none";
        if (step > 5) btnWrapper.style.display = "none";
        let details = "";
        const optionsInputCheck = document.querySelectorAll(".options__input:checked"), countCredit = document.querySelector("#count-credit");
    ...
    function trackContentChangeOnClick(element) {
        ...
        activeMap ? activeMap.classList.remove("map_active") : null;
        const numMap = element.dataset.value, curMap = document.querySelector(`[data-map="${numMap}"]`);
